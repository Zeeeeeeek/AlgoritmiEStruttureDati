...
public class Aula implements Comparable<Aula> {
    ...
    public boolean addFacility(Facility f) {
        ...
        if (facilities[facilities.length - 1] != null) Arrays.copyOf(facilities, facilities.length * 2);
    ...
    public void addPrenotazione(TimeSlot ts, String docente, String motivo) {
        if(ts == null || docente == null || motivo == null) throw new NullPointerException("ts o docente o motivo sono" +
                "null");
        if(isFree(ts)) {
            Prenotazione prenotazione = new Prenotazione(this, ts, docente, motivo);
        }
    }
